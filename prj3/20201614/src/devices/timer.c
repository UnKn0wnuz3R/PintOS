...
timer_sleep (int64_t ticks) 
{
  ...
  list_insert_ordered(&sleep_list,&thread_current()->elem,cmp_wakeup_tick,NULL);
  thread_block();
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  while(!list_empty(&sleep_list)){
    ...
    thread_ptr = list_entry(head,struct thread,elem);
    
    if(thread_ptr->wakeup_tick > ticks){
      break;
    }
    else{
      list_remove(head);
...
/* wakeup_tick이 이른 스레드 부터 앞쪽에 위치하도록 */
bool cmp_wakeup_tick(struct list_elem *first, struct list_elem *second, void *aux)
{
